...
class AlertHelpers(object):
    ...
    def setup_mail_server(self,splunk,user,password,mode='None'):
        """
        ...
        if(mode == 'SSL'):
            ...
            request_args['mailserver']=const.TestConstants['SMTP_GMAIL_SERVER'].format('465')
        ...
        if (mode == 'TLS'):
            request_args['auth_password'] = password
            request_args['auth_username'] = user
            request_args['mailserver']=const.TestConstants['SMTP_GMAIL_SERVER'].format('587')
...
#        request_args['action.email.sendresults'] = 'True'
    ...
    def deleteAlert(self,alertname):
        ...
        request_args=''
        make_http_request(request_type,request_url,request,args)
    ...
    def get_paper_orientation(self,splunk,alertname):
        ...
        content = self.make_http_request(splunk,request_type,request_url,req_args)
    ...
    def get_triggered_alert_count(self,splunk,alert_name):
        ...
        content = self.make_http_request(splunk,request_type,request_url,req_args)
        parsedresponse = json.loads(content)
        triggered_alert_count = str(parsedresponse['entry'][0]['content']['triggered_alert_count'])
        return triggered_alert_count
    ...
    def create_new_app(self,splunk,appname,appcontext,user):
#        app_params = {'name':'nithya','app':'search','user':'admin',}
        ...
        request_type = 'POST'
        self.make_http_request(splunk,request_type,app_url,app_params)
    
    def get_splunk(self,TEST_DIR,branch):
        '''
        Splunk  instance
        '''
        splunk_home = TEST_DIR + os.sep + 'test_installs' + os.sep + 'temp_splunk_instance'
        splunk_instance = SplunkFactory.getSplunk(splunk_home)
#        LOGGER.info("Installing Heavy Forwarder from nightly")
        splunk_instance.install_nightly(branch=branch)
        splunk_instance.COMMON_FLAGS = splunk_instance.COMMON_FLAGS + ' --auto-ports'
        splunk_instance.restart()

        return splunk_instance
    
    def one_shot_upload(self,splunk,user,appcontext,one_shot_file):
        ...
        request_type = 'POST'
        self.make_http_request(splunk,request_type,oneshot_url,oneshot_args)
